...
public partial class Templatestorepdf : System.Web.UI.Page
{
    ...
    protected void lnk_openIE_New_Click(object sender, EventArgs e)
    {
        ...
        else if (TreeView1.CheckedNodes.Count == 1)
        {
            ...
            {
                if (name == "NF-KDV Medical.pdf" || name == "WC packet.pdf" || name == "WC-KDV Medical.pdf" || name == "Work Letter.pdf" || name == "Pending Sx letter.pdf")
        ...
        else if (TreeView1.CheckedNodes.Count == 1)
        {
            ...
            if (TreeView1.CheckedNodes.Count > 0 && TreeView1.CheckedNodes.Count < 2)
            {
                ...
                if (!string.IsNullOrEmpty(fullName))
                {
                    ...
                    //{
                    if (childfilename.Text.Contains("NF-KDV Medical1234.pdf"))
                    {
                    ...
                    //{
                     //   string newfilename = childfilename.Text;
                     //   float[] xaxis = new float[] { 400f, 0f, 0f, 0f, 0f,380f,380f,280f,100f };
                     //   float[] yaxis = new float[] { 325f, 0f, 0f, 0f, 0f,225f,500f,325f,80f };
                     //   //string imagepath = Server.MapPath("~/Sign/4639_768.jpg");
                     //   string imagepath = fullName;
                     //   setPDF(pdfPath, newfilename, imagepath, xaxis, yaxis);
                     //   pdfPath = Server.MapPath("~/PdfForms/" + newfilename);
                   // }
                    ...
                    //{
                     //   string newfilename = childfilename.Text;
                      //  float[] xaxis = new float[] { 0f, 250f, 100f };
                      //  float[] yaxis = new float[] { 0f, 410f, 80f };
                      //  //string imagepath = Server.MapPath("~/Sign/4639_768.jpg");
                      //  string imagepath = fullName;
                      ////  setPDF(pdfPath, newfilename, imagepath, xaxis, yaxis);
                       // pdfPath = Server.MapPath("~/PdfForms/" + newfilename);
                ...
                {
                    formFieldMap["txtDoctor"] = txtDoctor.Text;
                }
                else
                { formFieldMap["txtDoctor"] = "Dr. Upendra Sinha"; }
                ...
                formFieldMap["txt_LocAddres"] = Convert.ToString( Session["LocAddres"]);
                ...
                if (string.IsNullOrWhiteSpace(Convert.ToString(Session["AGE"])))
                {
                ...
                formFieldMap["txt_admitting_surgeon"] = " Dr. Upendra Sinha";
                formFieldMap["txt_admitting_surgeon_add"] = " 102-31 Jamaica Ave., Richmond Hill, NY 11418";
           



                formFieldMap["txtEmpName"] = Convert.ToString(Session["EmpName"]);
                formFieldMap["txtEmpAddress"] = Convert.ToString(Session["EmpAddress"]);
                formFieldMap["txtEmpNameAdd"] = Convert.ToString(Session["EmpName"]) + " ; " + Convert.ToString(Session["EmpAddress"]);
                formFieldMap["txt_WCBGroup"] = Convert.ToString(Session["WCBGroup"]);
                formFieldMap["txt_Patientfax"] = "646-737-0176";
     			formFieldMap["txt_tel_phno_b"] = "718-480-1130";
                formFieldMap["txt_npino"] = "1265471726";
                formFieldMap["txt_emaldefault"] = "wcbclaimsfiling@wcb.ny.gov";
                

                formFieldMap["txt_guid_c_2"] = "D";
				formFieldMap["txt_guid_c_3"] = "6";
				formFieldMap["txt_guid_c_4"] = "F";

                formFieldMap["txt_WCB_1"] = "1";
                formFieldMap["txt_WCB_2"] = "6";
                formFieldMap["txt_WCB_3"] = "6";
                formFieldMap["txt_WCB_4"] = "0";
                formFieldMap["txt_WCB_5"] = "3";
                formFieldMap["txt_WCB_6"] = "0";
                formFieldMap["txt_WCB_7"] = "7";
                formFieldMap["txt_WCB_8"] = "W";
    ...
    public void bindEditData(string PatientIEid)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //if (string.IsNullOrWhiteSpace(ds.Tables[0].Rows[0]["LocAddres"].ToString()))
                //{
                 //   Session["LocAddres"] = " ";
                //}
                //else
               // {
               //     Session["LocAddres"] = Convert.ToString(ds.Tables[0].Rows[0]["LocAddres"])+" "+ Convert.ToString(ds.Tables[0].Rows[0]["LocCity"])+" "+Convert.ToString(ds.Tables[0].Rows[0]["LocState"]);
               // }
    ...
    public void setPDF(string pdfpath, string pdfpathourput1, string imgpath, float[] x, float[] y)
    {
        ...
        using (Stream outputPdfStream = new FileStream(pdfpathourput, FileMode.Create, FileAccess.Write, FileShare.None))
        {
            ...
            string temp = imgpath.Split('\\')[4];
            string fnmae = string.Empty;//= imgpath;
            if (!string.IsNullOrEmpty(temp))
            { fnmae = Server.MapPath("~/Sign/" + temp); }
     ...
     protected void lnk_openIEPdf_Click(object sender, EventArgs e)
    {
        if (TreeView1.CheckedNodes.Count > 0 && TreeView1.CheckedNodes.Count < 2)
        {
          ...
          if (File.Exists(Server.MapPath("~/TemplateStore/DownloadPdf/" + filename)))
            {
                //   pg.Stamping(Server.MapPath("~/TemplateStore/DownloadPdf/" + filename), "PatientIE_ID", id);
